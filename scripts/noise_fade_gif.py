...
# Configuration
input_files_location = "../frog_pfps/cropped_gifs"
output_files_location = "../frog_pfps/postprocess"
...
def process_gif(input_path, output_path):
    """
    Processes each frame of the GIF: adds noise and applies a fading effect.
    """
    with Image.open(input_path) as gif:
        ...
        for frame in ImageSequence.Iterator(gif):
            ...
            final_frame = frame_with_fade.convert("P", palette=Image.ADAPTIVE, dither=None)
...
for filename in os.listdir(input_files_location):
    if filename.endswith(file_types):
        ...
        output_path = os.path.join(output_files_location, "processed2_" + filename)  # To ensure unique output filename
