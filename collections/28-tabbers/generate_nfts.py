...
from code.scripts.svgizer import png_to_svg_file  # ← NEW
...
def main():
    ...
    # NEW: SVG output path (defaults to media/svgs)
    output_svgs_path = config.get('output_svgs_path', 'media/svgs')

    # Optional SVG knobs (safe defaults)
    svg_scale = int(config.get('svg_scale', 12))
    svg_alpha_threshold = int(config.get('svg_alpha_threshold', 1))
    svg_include_bg = bool(config.get('svg_include_bg_rect', False))
    svg_bg_color = config.get('svg_bg_color', '#000000')
    svg_filter_id = config.get('svg_filter_id', None)
    collection_name = config.get('collection_name', 'Frogs')
    ...
    os.makedirs(output_svgs_path, exist_ok=True)  # ← NEW
    ...
    for image_number in range(start_nft_number, end_nft_number + 1):
        ...
        # NEW: Build SVG into media/svgs (or output_svgs_path from config)
        try:
            svg_path = os.path.join(output_svgs_path, f"{image_number}.svg")
            png_to_svg_file(
                input_png=final_image_path,
                output_svg=svg_path,
                scale=svg_scale,
                alpha_threshold=svg_alpha_threshold,
                include_bg_rect=svg_include_bg,
                bg_color=svg_bg_color,
                filter_id=svg_filter_id,
                title=f"{collection_name} #{image_number}",
                desc="On-chain pixel SVG generated from composite PNG"
            )
            print(f"🧩 SVG saved → {svg_path}")
        except Exception as e:
            print(f"⚠️ SVG generation failed for #{image_number}: {e}")
